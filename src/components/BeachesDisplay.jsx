...
const BeachesDisplay = ({ data, beachesData }) => {
    const navigate = useNavigate();

    const handleNavigation = (id) => {
        navigate(`/where-to-go/BEACHES/${id}`);
    };
        ...
        <div>
            <Box className="flex flex-col gap-[40px]  " >
                <Box className=" flex flex-col gap-[10px] mt-[30px]">
                    <Text>{beachesData?.length} POSTS IN</Text>
                    <h1 className=' font-[900] text-[30px]'>The Most Beautiful Beaches in the World</h1>
                    <Text style={{ backgroundColor: 'rgb(18 98 175)' }} className='px-[20px] py-[5px] text-white text-[13px] font-[700] rounded-[8px] w-fit'>BEACHES</Text>

                    <Box className="flex flex-row flex-wrap gap-[10px]">

                        { beachesData?.map((x, index) => (
                            <div
